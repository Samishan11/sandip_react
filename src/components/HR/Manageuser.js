      ...
      <div className="content w-100">
        <div className=''>
          <div className='my-4 px-4'>
            <h4 className="text-light">Manage User</h4>
            <small className='text-sm text-light'>See all user available for your company.</small>
          </div>
          <div className='container-fluid px-4'>
            <div className="container mx-auto row border rounded position-relative bg-light shadow-sm py-4" style={{ zIndex: "1" }}>
              <div className="col-md-4">
                ...
                <input onChange={e => setfilterchange(e.target.value)} type="text" className="form-control form-control-sm form-control-solid" placeholder="Search..." />
              </div>
              <div className="col-md-4">
                <label htmlFor="">User Type</label>
                <select onChange={e => setfilterchange(e.target.value)} className="form-control-sm form-select py-0 text-secondary text-sm" name="" id="">
                  <option className="text-sm" disabled value="" selected>None Selected</option>
                  <option className="text-sm" value={'isHR'}>Hr</option>
                  <option className="text-sm" value={'isManager'}>Manager</option>
                  <option className="text-sm" value={'isEmployee'}>Employee</option>
                </select>
              </div>
              <div className="col d-flex align-items-end">
                <button onClick={() => {
                  searchUser()
                }} className="btn btn-sm btn-primary w-75">Filter</button>
              </div>
            </div>
          </div>
              ...
              <div className='col'>
                ...
                <div className='mb-4'>
                  <div className="d-flex align-items-center my-3">
                    ...
                    <Link className="ms-auto" to="/add-user">
                      ...
                      getUser?.map((val, ind) => {
                        return <div key={ind} className='border rounded shadow-sm row align-items-center my-2 mx-0 py-2 job-row'>
                          <div className='col-3 d-flex align-items-center'>
                            {
                              val?.profile_pic ?
                                <div className='no-img-avatar-sm' ><img className='no-img-avatar-sm' src={`http://localhost:5000/${val?.profile_pic}`} /></div> :
                                <div className='no-img-avatar-sm'>{val?.firstName?.slice(0, 1)?.toUpperCase() + val?.lastName?.slice(0, 1)?.toUpperCase()}</div>
                            }
                            <div className='mx-2'>
                          ...
                          <div className='col'>
                            <p className='m-0 badge badge-muted-primary fw-light'>{val?.username}</p>
                          </div>
                          <div className='col'>
